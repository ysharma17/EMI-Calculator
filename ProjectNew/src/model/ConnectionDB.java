...
public class ConnectionDB {
    ...
    public ResultSet Read(String loanId,String date,Connection con)
    {
		...
		try 
		{
			...
			rs = stmt.executeQuery("select * from loanschedule where loanid='"+loanId+"'and dueda" +
									"te<='"+date+"' and status=(select max(status) from loanschdd" +
									"etails where loanid='"+loanId+"') and clbal=(select min(clbal)" +
									" from loanschddetails where loanid='"+loanId+"' and duedate<='"+date+"') " +
    ...
    public ResultSet Read(String loanId,int instNo,Connection con)
    {
		...
		try 
		{
			...
			rs = stmt.executeQuery("select * from loanschedule where loanid='"+loanId+"'and instno='"+instNo+"'" +
							" and status=(select max(status) from loanschddetails where loanid='"+loanId+"')");
    ...
    public ResultSet ReadLoanId(Connection con,String custId,int request)
    {
		...
		{
			Statement stmt=con.createStatement();
			rs = stmt.executeQuery("select loanid from "+tableName+" where custid='"+custId+"' and  status!=0");
		} 
		catch (SQLException e) 
		{
			e.printStackTrace();
    ...
    public ResultSet ReadLoanIdAll(Connection con,int request)
    {
		...
		{
			Statement stmt=con.createStatement();
			rs = stmt.executeQuery("select loanid from "+tableName+" where status!=0");
		} 
		catch (SQLException e) 
		{
			e.printStackTrace();
...
//    public void Write(CustLoanDetails cl,Connection con,int request)
//    {
//
//    	String tableName=null;
//    	if(request==1)
//    	{
//    		tableName="TEMPCustLoanDetails";
//    	}
//    	else
//    	{
//    		tableName="CustLoanDetails";
//    	}
//    
//        try {
//            PreparedStatement pstmt=con.prepareStatement("insert into "+tableName+" (LoanId,custId ,prepaypen) values(?,?,?)");
//          
//            pstmt.setString(1,cl.getLoanID());
//            pstmt.setString(2,cl.getCustID());
//            pstmt.setDouble(3,cl.getPrePayPen());
//            int nRows=pstmt.executeUpdate();
//            System.out.println(nRows+"inserted");
//            pstmt.close();
//            
...
//    
    public int Write(LoanSchedule lsd,Connection con,int request){
    	...
    	if(request==1)
    	{
    		tableName="TEMPLOANSCHDDETAILS";
    	}
    	else
    	{
    		tableName="LOANSCHDDETAILS";
    	}
        try 
        {
    ...
    public void Write(CustDetail cd,Connection con){
        ...
        try {
            PreparedStatement pstmt=con.prepareStatement("insert into CustRegistration (CUSTID,USERNAME," +
...
//    public void Write(CustLogin cl,Connection con){
//
//        try {
//            PreparedStatement pstmt=con.prepareStatement("insert into CustLogin (CUSTID,USERNAME,USERPWD,DESIGNATION) values(?,?,?,?)");
//            
//            pstmt.setString(1,cl.getCustId());
//            pstmt.setString(2,cl.getUserName());
//            pstmt.setString(3, cl.getUserPwd());
//            pstmt.setString(4,cl.getDesignation());
//            int nRows=pstmt.executeUpdate();
//            System.out.println(nRows+"inserted");
//            pstmt.close();
//            
...
//    }
    ...
    public void Write(String loanId,Connection con)
    {
    	try {
            ...
            String str="update loandetail set status=0 where loanid='"+loanId+"' and status=(select max(Status)" +
            						" from loanschddetails where loanid='"+loanId+"')";
    ...
    public void Write(double ppp,String type,Connection con)
    {
    	try {
            ...
            String str="update penalty set prepaypen="+ppp+" where type='"+type+"'";
            int nRows=stmt.executeUpdate(str);
            System.out.println(nRows+"  updated");
            stmt.close();
